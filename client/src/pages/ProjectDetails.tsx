...
export const ProjectDetails: React.FC = () => {
    ...
    const [editStatus, setEditStatus] = useState<
        "in-progress" | "critical" | "on-hold" | "completed"
    >("in-progress");
    ...
    const handleRemoveContributor = (nameToRemove: string) => {
        if (
            confirm(
                `هل أنت تأكد من إزالة (${nameToRemove}) من فريق عمل المشروع؟`,
            )
        ) {
            setContributors(
                contributors.filter((c) => c.name !== nameToRemove),
            );
        }
    };
            ...
            {isEditProjectModalOpen && (
                <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center z-50 p-4">
                    <div className="bg-white border border-slate-200 rounded-md p-6 w-full max-w-md space-y-4">
                        ...
                        <form
                            onSubmit={handleSaveProjectEdits}
                    ...
                    <div className="bg-white border border-slate-200 rounded-md p-6 w-full max-w-md space-y-4 text-xs">
                        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                            <h2 className="text-base font-bold text-slate-900">
                                إدارة مساهمي مشروع ({project?.title})
                            </h2>
                            <button
                                onClick={() =>
                        ...
                        <div className="divide-y divide-slate-100 space-y-2">
                            {contributors.map((c) => (
                                ...
                                >
                                    ...
                                    <button
                                        onClick={() =>
                                            handleRemoveContributor(c.name)
                                        }
                        ...
                        >
                            <div className="space-y-1">
                                ...
                                <input
                                    ...
                                    onChange={(e) =>
                                        setMemberLeadName(e.target.value)
                                    }
                                    className="w-full border border-slate-300 rounded-md p-2 focus:border-blue-600 focus:outline-none"
